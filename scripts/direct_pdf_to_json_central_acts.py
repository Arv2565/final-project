"""
Direct PDF to JSON converter for Central Acts using Gemini API.
...
# Configuration
CENTRAL_ACTS_DIR = Path("/Users/pranav/Documents/Projects/final-project/data/knowledge_base/central_acts")
OUTPUT_JSON_PATH = Path("/Users/pranav/Documents/Projects/final-project/data/knowledge_base/central_acts.json")
ERROR_LOG_PATH = Path("/Users/pranav/Documents/Projects/final-project/data/knowledge_base/central_acts_processing_errors.log")
SUMMARY_REPORT_PATH = Path("/Users/pranav/Documents/Projects/final-project/data/knowledge_base/central_acts_processing_summary.txt")
MAX_PAGES_IGNORE = 50
...
def process_pdf_with_gemini(text_content: str, images: List[str], pdf_filename: str) -> Optional[Dict]:
    ...
    try:
        ...
        if text_content:
            # Text-based PDF
            prompt = f"""You are analyzing a Central Indian legal act document: {pdf_filename}
...
Return ONLY valid JSON, no markdown, no explanations."""
            ...
            prompt = f"""You are analyzing a scanned image of a Central Indian legal act: {pdf_filename}
...
def process_all_pdfs(limit: Optional[int] = None, dry_run: bool = False, skip_existing: bool = True):
    """
    Process all PDFs in the central_acts directory.
    ...
    """
    logger.info(f"Starting PDF processing from {CENTRAL_ACTS_DIR}")
    ...
    # Get all PDFs
    pdf_files = sorted(CENTRAL_ACTS_DIR.glob("*.pdf"))
    ...
    summary_lines = [
        "=" * 80,
        "PDF PROCESSING SUMMARY REPORT - CENTRAL ACTS",
        "=" * 80,
        f"Timestamp: {pd.Timestamp.now()}",
        f"Input directory: {CENTRAL_ACTS_DIR}",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert Central Acts PDFs to JSON format")
